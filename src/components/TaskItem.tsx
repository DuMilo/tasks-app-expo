...
const TaskItem: React.FC<TaskItemProps> = ({ task }) => {
  const startEditingTask = useTaskStore((state) => state.startEditingTask);
      ...
      <View style={styles.contentContainer}>
        ...
        {task.dueDate && (
          <Text style={[styles.dateText, isOverdue ? styles.dateOverdue : styles.dateOnTime]}>
            Ate: {new Date(task.dueDate).toLocaleDateString()}
          </Text>
        )}
      </View>
      <View style={styles.icons}>
        <TouchableOpacity onPress={() => startEditingTask(task)} accessibilityRole="button">
